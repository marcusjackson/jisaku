...
function mapRowToKanji(row: KanjiRow): Kanji {
  return {
    ...
    kenteiLevel: row.kanji_kentei_level as Kanji['kenteiLevel'],
...
export interface UseKanjiRepository {
  ...
  /** Update Kentei level */
  updateKenteiLevel: (id: number, level: KanjiKenteiLevel | null) => Kanji
  /** Update radical ID */
...
export function useKanjiRepository(): UseKanjiRepository {
  ...
  function search(filters?: KanjiFilters): Kanji[] {
    ...
    if (filters.searchKeywords) {
      // Search in both short_meaning and search_keywords fields
  ...
  function updateKenteiLevel(
    id: number,
    level: KanjiKenteiLevel | null
  ): Kanji {
