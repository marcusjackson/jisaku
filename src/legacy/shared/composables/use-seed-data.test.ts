 ...
 * useSeedData tests
 *
 * Tests for the shared seed data composable.
 */
...
describe('useSeedData', () => {
  ...
  describe('seed', () => {
    ...
    it('inserts seed data when database is empty', async () => {
      ...
      // Should insert 8 position types + 18 kanji + 8 components + 5 vocabulary + 0 component_occurrences (depends on mocks) = 39 calls
      // With current mocks: no component_occurrences/vocab_kanji because lookups don't return IDs
    ...
    it('shows success message after seeding', async () => {
      ...
      expect(mockShowSuccess).toHaveBeenCalledWith(
        'Seeded 8 position types, 18 kanji, 8 components, 5 vocabulary'
      )
    })
    ...
    it('exports seed data counts', () => {
      resetMocks()
      const { seedDataCounts } = useSeedData()

      expect(seedDataCounts).toEqual({
        kanji: 18,
        components: 8,
        positionTypes: 8,
        onReadings: 21,
  ...
  describe('clear', () => {
    ...
    it('prevents concurrent clearing', async () => {
      ...
      // run should only be called fourteen times (once for each table: 7 original + 2 readings + 3 meanings tables + 2 vocab tables)
