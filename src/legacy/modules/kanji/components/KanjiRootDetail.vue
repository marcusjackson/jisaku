...
import { useClassificationRepository } from '@/legacy/shared/composables/use-classification-repository'
import { useComponentOccurrenceRepository } from '@/legacy/shared/composables/use-component-occurrence-repository'
import { useComponentRepository } from '@/legacy/shared/composables/use-component-repository'
...
import { useMeaningRepository } from '@/legacy/shared/composables/use-meaning-repository'
import { useRadicalRepository } from '@/legacy/shared/composables/use-radical-repository'
import { useReadingRepository } from '@/legacy/shared/composables/use-reading-repository'
...
import type { QuickCreateComponentData } from '@/legacy/shared/validation/quick-create-component-schema'
...
async function handleDelete() {
  ...
  try {
    ...
    await router.push({ name: 'legacy-kanji-list' })
  } catch (err) {
    fetchError.value = err instanceof Error ? err : new Error(String(err))
  ...
  try {
    switch (field) {
      ...
      case 'joyoLevel':
        ...
        break
      case 'kenteiLevel':
        kanji.value = updateKenteiLevel(
          kanji.value.id,
          value as KanjiKenteiLevel | null
        )
        break
      case 'radicalId': {
