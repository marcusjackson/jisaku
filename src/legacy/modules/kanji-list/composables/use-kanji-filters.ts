...
function parseKenteiLevels(value: unknown): KanjiKenteiLevel[] | undefined {
  if (typeof value === 'string' && value.length > 0) {
    const validLevels: KanjiKenteiLevel[] = [
      '10',
      '9',
      '8',
      '7',
    ...
    const levels = value
      .split(',')
      .filter((v): v is KanjiKenteiLevel =>
        validLevels.includes(v as KanjiKenteiLevel)
      )
    return levels.length > 0 ? levels : undefined
  }
  return undefined
