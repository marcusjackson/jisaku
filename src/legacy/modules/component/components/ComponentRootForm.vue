...
import type { ComboboxOption } from '@/legacy/base/components/BaseCombobox.vue'
...
const backUrl = computed(() => {
  if (props.mode === 'edit' && !Number.isNaN(componentId.value)) {
    return `/legacy/components/${String(componentId.value)}`
  }
...
async function handleSubmit(values: ComponentFormData) {
  try {
    if (props.mode === 'create') {
      ...
      await router.push(`/legacy/components/${String(newComponent.id)}`)
    } else {
      ...
      await router.push(`/legacy/components/${String(updated.id)}`)
    }
  } catch (err) {
