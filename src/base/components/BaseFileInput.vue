...
<template>
  <div class="base-file-input">
    ...
    <!-- Hidden file input — lives outside the button to avoid interactive nesting -->
    <input
      :id="fileInputId"
      ref="fileInputRef"
    ...
    <!-- Wrapper provides the stacking context for the remove button overlay -->
    <div class="base-file-input-drop-zone-wrapper">
      <button
      ...
      >
        ...
        >
          ...
          <p class="base-file-input-text">Drag and drop or click to browse</p>
        </div>
      </button>

      <!-- Remove button: sibling of the drop zone button, overlaid via absolute positioning -->
      <button
        v-if="hasPreview"
...
.base-file-input-drop-zone-wrapper {
  position: relative;
}

.base-file-input-drop-zone {
  /* Reset native button styles, then drop zone styles in standard property order */
  appearance: none;
  display: flex;
...
.base-file-input-drop-zone:hover:not(:disabled) {
...
.base-file-input-drop-zone:focus-visible {
...
.base-file-input-remove:focus-visible {
