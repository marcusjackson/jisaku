/**
 * KanjiDetailStrokeOrderDisplay.test.ts
 *
 * Unit tests for KanjiDetailStrokeOrderDisplay component.
...
import KanjiDetailStrokeOrderDisplay from './KanjiDetailStrokeOrderDisplay.vue'

// Mock useBlobUrl to return computed ref with predictable URLs
vi.mock('@/base/composables/use-blob-url', () => ({
  useBlobUrl: vi.fn(
...
describe('KanjiDetailStrokeOrderDisplay', () => {
  describe('rendering', () => {
    it('renders empty state when no images provided', () => {
      render(KanjiDetailStrokeOrderDisplay, {
        props: {
          strokeDiagram: null,
          strokeGif: null
        }
