/**
 * Tests for SharedQuickCreateVocabulary component
 *
...
import SharedQuickCreateVocabulary from './SharedQuickCreateVocabulary.vue'

describe('SharedQuickCreateVocabulary', () => {
  // Mount with stubs to avoid portal issues
  function mountDialog(props: { open?: boolean; initialWord?: string } = {}) {
    return mount(SharedQuickCreateVocabulary, {
  ...
  it('pre-fills word when initialWord prop is provided', async () => {
    const wrapper = mountDialog({ initialWord: '食べる' })
    await wrapper.vm.$nextTick()
    const wordInput = wrapper.find('input[name="word"]')
    expect((wordInput.element as HTMLInputElement).value).toBe('食べる')
  })
  ...
  it('displays help text for kana field', () => {
    const wrapper = mountDialog()
  ...
  it('displays help text for short meaning field', () => {
    const wrapper = mountDialog()
    expect(wrapper.text()).toContain('Brief meaning for display (optional)')
  })
})
