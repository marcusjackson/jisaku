...
- Running this prompt constitutes implicit approval of requirements
- Spec.json will be updated to mark requirements as approved
...
3. **Validate File Size and Split if Necessary**:
   ...
   - **If content would exceed 600 lines**:
     1. Create a splitting plan organized by logical theme:
        - Common splits: `design-architecture.md`, `design-components.md`, `design-integrations.md`, `design-data-models.md`
        - Name files based on primary content theme (e.g., `design-frontend.md`, `design-backend.md` for stack-based splits)
     2. Create `design.md` as the index/overview:
        - Include feature overview and goals
        - List all design document parts with clear descriptions
        - Provide navigation links to all parts
        - Include high-level architecture summary
     3. Split content into themed files:
        - Each file MUST start with clear cross-references to other parts
        - Each file MUST be self-contained for its theme
        - Maintain consistent formatting and section structure
        - Keep related content together (don't split mid-component)
     4. Format all files with prettier/markdown formatter
     5. Verify no single file exceeds 600 lines after formatting
   - **If single file is acceptable** (under 600 lines):
     1. Write content to `design.md`
     2. Format with prettier/markdown formatter
     3. Re-check line count after formatting
     4. If formatted version exceeds 600 lines, trigger split process above

4. **Update Metadata** in spec.json:
   - Set `phase: "design-generated"`
   - Set `approvals.design.generated: true, approved: false`
   - Set `approvals.requirements.approved: true`
   - If design was split, add `design_files: ["design.md", "design-architecture.md", ...]` array
...
1. **Status**: Confirm design document(s) generated
   - Single file: `.kiro/specs/$1/design.md` (XXX lines)
   - Split files: `.kiro/specs/$1/design.md` (index) + themed parts listed
2. **Discovery Type**: Which discovery process was executed (full/light/minimal)
3. **Key Findings**: 2-3 critical insights from discovery that shaped the design
4. **File Organization** (if split): Brief description of how content was organized
